        ...
        @if (in_array($orderStatus->order_status_id, $hideStatusIds))
            ...
            <li class="active {{ $extraClass }}">
                <h6 class="mb-0">
                    ...
                    @endif
                    {{--@if($order->order_status >= 6 && $orderStatus->order_status_id == 6  && !empty($order->order_latter) && $order->order_status != 18 && $order->order_status != 19)
                ...
                <h6 class="mb-0 {{ $flexClass }}">
                    ...
                    @else
                        @if(in_array($orderItemCategoryId,\App\Models\Category::SERVICES_CATEGORY_IDS) && $order->order_status== 1 && ($orderStatus->order_status_id == 2 || $orderStatus->order_status_id == 11 || $orderStatus->order_status_id == 12) && ($orderlogisticProvided->logistic_check==1 && $orderlogisticProvided->logistic_provided ==0))
                            <a href="javascript:void(0)" class="orderStatusChange" id="orderStatusChange{{$order->id}}"  data-order-id="{{$order->id}}" data-status-id="{{$orderStatus->order_status_id}}">{{ __('order.' . trim($orderStatus->status_name)) }}</a>
                    ...
                    @endif
                    @if ($orderStatus->order_status_id == 3 && $order->payment_status==0)
                        <div class="px-2">
