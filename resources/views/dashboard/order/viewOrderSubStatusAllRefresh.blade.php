...
<tbody>
@php
    $quote_items = get_quote_items_order($order->quote_id);
    $countOrderItems = 0;
...
@if(!empty($quote_items))
    @foreach($quote_items as $key => $value)
        @php
            $orderItem = $value->orderItem()->first();
            $orderAirwayBill = $orderItem->orderAirwayBillNumber()->first();
            $i++;
            if (!empty($orderItem)){
                $countOrderItems += 1;
        ...
        @if(in_array($orderItemCategoryId,\App\Models\Category::SERVICES_CATEGORY_IDS) && ($orderlogisticProvided->logistic_check==1 && $orderlogisticProvided->logistic_provided ==0) && $i==1)
            <tr>
                {{view('dashboard/order/singleOrderItemStatusRefresh', ['orderItem'=>$orderItem,'orderAirwayBill'=>$orderAirwayBill,'orderItemCategory'=>$orderItemCategory,'orderItemCategoryId'=>$orderItemCategoryId,'orderlogisticProvided'=>$orderlogisticProvided])}}
            </tr>
        @endif
        <tr class="b-none" id="openProduct{{$orderItem->id}}">
            <td class="align-middle">
                <div class="d-flex">
                    ...
                    @endif
                        <span class="prod_name">{{ get_product_name_by_id($value->rfq_product_id, 1) }}</span>
                </div>
            </td>
            <td class="text-center text-nowrap  align-middle">{{ $value->product_quantity??'' }} {{ $value->unit_name??'' }}</td>
            <td class="text-center text-nowrap  align-middle">
                <span class="badge bg-primary" style="font-size: .7em;" id="orderItemStatusNameBlock{{$orderItem->id}}">{{ __('order.'.getOrderItemStatusNameById($orderItem->order_item_status_id)) }}</span>
            </td>
...
<tr class="bg-light">
    ...
    <td class="text-end text-nowrap fw-bold align-middle">
        @php
            ...
            $bulkOrderDiscount = getBulkOrderDiscount($order->id)??0;
        @endphp
