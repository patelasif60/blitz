...
<div class="modal-body">
    ...
    <div class="row rfqform_view g-3">
        ...
        <div class="col-md-4">
            ...
            <div>{{ $quote->supplier_company_name }}</div>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            @if($quote->payment_type==1)
                <div class="text-dark"><span class="badge rounded-pill bg-danger">{{ __('admin.credit') }} - {{$quote->credit_days}}</span></div>
            @elseif($quote->payment_type==0)
                ...
                <tr>
                    @if ($charges->type == 0)
                        @php
                            ...
                            $flagSuppler = 1;
                        @endphp
                        <td colspan="3">{{ $charges->charge_name . ' ' . $charges->charge_value }} % <small class="fw-bold text-blue">({{ __('admin.supplier_other_charges') }})</small></td>
                    @else
                        @if($charges->charge_type == 1)
                            @php
                                $flagLogistic = 1;
                            @endphp
                            <td colspan="3">{{ $charges->charge_name }} <small class="fw-bold text-blue">({{ __('admin.logistic_charges') }})</small></td>
                        @elseif($charges->charge_type == 2)
                            <td colspan="3">{{ $charges->charge_name }}</td>
                        @else
                            @php
                                $flagSuppler = 1;
                            @endphp
                            <td colspan="3">{{ $charges->charge_name }} <small class="fw-bold text-blue">({{ __('admin.supplier_other_charges') }})</small></td>
                        @endif
                    @endif
