...
<div class="col-md-6 pb-2">
    ...
    <div class="text-primary">{{ $rfq->created_by }}</div>
</div>
<div class="col-md-6 pb-2">
    <label>{{ __('rfqs.product') }}:</label>
    <div class="multipleproducthover">
        <div href="javascript:voic(0);" class="tooltiphtml w-auto">
            <span class="text-primary">{{ $rfq->all_products->count() }} {{ __('rfqs.products') }}</span>
...
@if(isset($rfq->rfq_attachments) && count($rfq->rfq_attachments)>=1)

    <div class="col-md-6 pb-2">
        <label>{{ __('rfqs.rfq_attachment') }}:</label>
        @php
            if(count($rfq->rfq_attachments)>1){
                 $downloadAttachment = "downloadAttachment(".$rfq->id.", 'attached_document','".$rfq->reference_number."')";
            }else{
                 $rfqFileTitle = Str::substr($rfq->rfq_attachments[0]->attached_document,44);
                 ...
                 $downloadAttachment = "downloadimg(".$rfq->rfq_attachments[0]->id.", 'attached_document', '".$rfq_file_name."')";
            }
        @endphp
        <div>
