                ...
                <ul class="nav nav-tabs bg-white newversiontabs ps-3" id="myTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link px-0 active" id="home-tab" data-bs-toggle="tab" data-bs-target="#home" type="button" role="tab" aria-controls="home" aria-selected="true">
                            {{__('admin.add_payment_term')}}
                        </button>
                    </li>
                </ul>
                <div class="tab-content pt-3 pb-0" id="myTabContent">
                    <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <form id="paymenttermadd" class="" method="POST" enctype="multipart/form-data" action="{{ route('payment-term-create') }}"
...
@section('scripts')
<script>
    $("#paymenttermadd").on('submit',function(event){
        event.preventDefault();
        var formData = new FormData($("#paymenttermadd")[0]);
        formData.append('description',tinyMCE.get('description').getContent());
        if ($('#paymenttermadd').parsley().validate()) {
            $.ajax({
                ...
                success: function (r) {
                    if(r.success == true){
                        ...
                        $.toast({
                            ...
                            text: "{{ __('admin.payment_term_added_alert')}}",
