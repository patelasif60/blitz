...
<div class="col-12 grid-margin ">
    <div class="row">
        <div class="col-md-12 d-flex align-items-center mb-3">
            <h1 class="mb-0 h3">{{ __('admin.designation')}}</h1>
        ...
        <div class="col-12">
            ...
            <div class="tab-content pt-3 pb-1" id="myTabContent">
                <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                    <form id="designationedit" class="" method="POST" enctype="multipart/form-data" action="{{ route('designation-update') }}" data-parsley-validate>
                        @csrf
                            ...
                            <h3 class="card-title">Edit Designation</h3> -->
                                        <div class="row">
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <input type="text" class="form-control" id="name" name="name" required value="{{ $designation->name }}">
                                            </div>
                                            <div class="col-md-6 mb-3">
                                                <label for="status" class="form-label">{{ __('admin.status')}}</label>
                                                <div>
                                                <input type="radio" value="1" name="status" {{ $designation->status == 1 ? 'checked' : '' }}>
                                                    {{ __('admin.active')}}
                                                <input type="radio" value="0" name="status" {{ $designation->status == 0 ? 'checked' : '' }}>
                                                    {{ __('admin.deactive')}}
                                                </div>
                                            </div>
                                        </div>
...
@section('scripts')
<script>
    $("#designationedit").on('submit', function(event) {
        event.preventDefault();
        var formData = new FormData($("#designationedit")[0]);
        if ($('#designationedit').parsley().validate()) {
            $.ajax({
                ...
                success: function(r) {
                    if (r.success == true) {
                        ...
                        $.toast({
                            ...
                            text: "{{__('admin.designation_updated_alert')}}",
