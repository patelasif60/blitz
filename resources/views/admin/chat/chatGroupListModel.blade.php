                        ...
                        @if(Auth::user()->hasRole('admin'))
                            @if(!$groupChatList->isEmpty())
                                @foreach($groupChatList as $groupChat)
                                    <div class="rfq-name d-flex align-items-center mb-3 p-2" id="{{ $groupChat['group_name'] }}" onclick="chat.chatAdminViewData('{{ route('get-chat-data-ajax')  }}', '{{$groupChat['_id']}}', '{{$header_name}}','{{$groupChat['group_name']}}', '{{$groupChat['chat_id']}}', $(this),'{{$groupChat['company_id']??''}}') ">
                        ...
                        @if(Auth::user()->hasRole('supplier'))
                            @if(!$supplierRfq->isEmpty())
                                @foreach($supplierRfq as  $supplier)
                                    ...
                                    @endphp
                                    <div class="rfq-name d-flex align-items-center mb-3 p-2" id="{{ $group_name }}" onclick="chat.chatAdminViewData('{{ route('get-chat-data-ajax')  }}', '{{$id}}', '{{$header_name}}','{{$group_name}}', '{{$chat_id}}',$(this),'{{$company_id}}') ">
                                        <a href="javascript:void(0)" class="nav-link p-0 rfq-contact text-dark">
