<div class="card border-0 rounded-0">
    <div class="middle-section d-flex align-items-center p-3">
        <div class="middle-message-name d-flex align-items-center">
            <div class="RFQ-Name mt-2 fw-bold" style="cursor: pointer;">{{$header_group_name}}</div>
        </div>
    </div>
    <div class="chat-content p-3 pb-1">
        ...
        <div class="chatbodytext mb-2 pe-2">
            ...
            @foreach($allMessage as $message)
                @php
                    ...
                    }elseif($message->sender_role_id == 1){
                        $name = 'blitznet Team';
                ...
                @if($message->sender_id != auth()->user()->id)
                    <div class="card chat-message {{$class}} mb-2">
                        <div class="card-body p-1 px-2">
                            <div class="messanger-name fw-bold mb-0">{{$name}}</div>
        ...
        <div class="chat-type-content mt-auto">
            <div class="input-group">
                <input type="text" id="chatInput_Support" class="form-control p-1 chat-type mb-0" onkeypress="chat.sendSupportMessageBackend(event, this, '{{ route("support-chat-create-backend-ajax") }}','{{ $id }}', '{{ $header_group_name }}', '{{ Auth::user()->id }}', '{{ Auth::user()->role_id }}', '{{ $company_id??0 }}')" placeholder="{{ __('admin.reply_here') }}" aria-label="Username" aria-describedby="basic-addon1" style="height: 36px;">
            </div>
            ...
            <div class="multiple-buttons d-flex justify-content-start align-items-center px-3 py-2">
                    ...
                    <div class="accordion accordion-flush  d-flex chat-accordion" id="chataccordionFlush">
                        <div class="accordion-item">
                            ...
                            <div id="flush-collapseOne" class="accordion-collapse collapse smileysection" aria-labelledby="flush-headingOne" data-bs-parent="#chataccordionFlush">
                                <div class="accordion-body p-0" id="smiley_Support"></div>
                            </div>
                        </div>
                        <div class="accordion-item">
                ...
                <a href="javascript:void(0)" class="ms-auto" onclick="chat.sendSupportMessageBackendOnClick(event, this, '{{ route("support-chat-create-backend-ajax") }}','{{ $id }}', '{{ $header_group_name }}', '{{ Auth::user()->id }}', '{{ Auth::user()->role_id }}', '{{ $company_id }}')">
...
<script>
    ...
    function showEmoji() {
        ...
        $('#smiley_Support').html(emoji);
    }

    function getEmoji(control) {
        document.getElementById('chatInput_Support').value += control.innerHTML;
        $('#chatInput_Support').focus().trigger('click');
    }
    ...
    $('#showEmojiPanel').on('click', function (e) {
        ...
        $('#smiley_Support').toggleClass("show");
