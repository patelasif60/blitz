...
<body style="page-break-inside: avoid;">
    ...
    <table width="100%" cellpadding="5" cellspecing="0" style="page-break-inside: avoid;font-family: Arial, Helvetica, sans-serif;">
        <tr>
            <td>
                <table width="100%" cellpadding="5" cellspecing="0" style="page-break-inside: avoid;font-family: Arial, Helvetica, sans-serif;">
                    <tr>
                            ...
                            <table width="90%" cellpadding="5">
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <p><strong>{{ __('admin.phone_number') }}:</strong> {{ countryCodeFormat($supplier->cp_phone_code,$supplier->contact_person_phone) }}</p>
                                        <p><strong>{{ __('signup.email') }}:</strong> {{ $supplier->contact_person_email }}</p>
                                    </td>
                                </tr>
                            </table>
        ...
        @php
            $finalAmount = $order->product_amount;

            $discount_object = $quotes_charges_with_amounts
                ->filter(function ($item) {
                    return strpos(strtolower($item->charge_name), 'discount') !== false;
                })
                ->first();
        @endphp
        <tr>
                ...
                <table width="100%" cellpadding="5" cellspecing="0" border="0">
                    ...
                    @foreach ($quotes_charges_with_amounts as $charges)
                        <tr>
                            ...
                            @else
                                <td colspan="5" style="text-align: right; border-bottom: 1px solid #e6e4e4; font-size: 12px;">@if($charges->custom_charge_name) {{$charges->custom_charge_name.' -'}} @endif {{ $charges->charge_name }}</td>
                            @endif
                    ...
                    <tr>
                        ...
                        <td align="right" style="text-align: right; border-bottom: 1px solid #e6e4e4; font-size: 12px;">
                            + Rp {{ number_format($quote->supplier_tex_value, 2) }}
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="text-align: right; border-top: 1px solid #000; border-bottom: 1px solid #000;  background-color: #f4f4f4; font-size: 12px; white-space: nowrap;">
                            <strong>Rp {{number_format($quote->supplier_final_amount, 2)}}</strong>
                        </td>
                    </tr>
                    </tfoot>
                </table>
